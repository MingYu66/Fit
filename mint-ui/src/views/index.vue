  ...
  <div class="app-container home">
    ...
    <el-card class="box-card" shadow="hover">
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="4"  v-for="item in userTypes">
          <div>
            <el-statistic :title="item.type">
    ...
    <div style="display: flex;justify-content:space-between ;margin-top: 10px;">
      ...
      <el-card class="box-card" :span="6" style="width: 49%;">
        <div slot="header" class="clearfix">
          <span>用户体重监测</span>
          <el-select style="float: right; padding: 3px 0" v-model="userIndex" @change="getUserXtById()" placeholder="请选择">
            <el-option v-for="item,index in boheUserList" :key="index" :label="item.name" :value="index">
            </el-option>
          </el-select>
        </div>
      ...
      <el-card class="box-card" :span="6" style="width: 49%;">
        ...
        <div id="bingtu" style="width: 100%;height: 350%"></div>
...
import { getUserWeight, getWeight, delWeight, addWeight, updateWeight } from "@/api/system/weight";
import { listBoheUser, selectUserType, delBoheUser, addBoheUser, updateBoheUser } from "@/api/system/boheUser";
...
export default {
  ...
  created() {
    ...
    listBoheUser({
      ...
      var data = { userId: this.boheUserList[this.userIndex].id }

      getUserWeight(data).then((res) => {
        console.log(res)
        if (res.data.length < 2 && this.userIndex < this.boheUserList.length - 1) {
          ...
          for (const item of res.data) {
            list.push(item.weight)
            days.push(item.createTime)
          }
          this.option.series[0].data = list
          this.option.xAxis[0].data = days
    ...
    selectUserType().then((res)=>{
      console.log(res)
      this.userTypes = res.data
      this.userSize = 0
      var map = []
      for (const iterator of this.userTypes) {
        this.userSize = iterator.id + this.userSize
        map.push({value:iterator.id,name:iterator.type})
      }
      this.bingtu.series[0].data = map
    })
  ...
  methods: {
    ...
    setDom() {
      ...
      myChart2.setOption(this.bingtu);
      if(this.boheUserList.length > 0){
        this.userId = this.boheUserList[this.userIndex]
      }
    },
    goTarget(href) {
